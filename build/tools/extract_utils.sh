...
function setup_vendor() {
    ...
    export SHIRAYUKI_ROOT="$3"
    if [ ! -d "$SHIRAYUKI_ROOT" ]; then
        echo "\$SHIRAYUKI_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SHIRAYUKI_ROOT/$OUTDIR" ]; then
        mkdir -p "$SHIRAYUKI_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SHIRAYUKI_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SHIRAYUKI_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SHIRAYUKI_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SHIRAYUKI_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SHIRAYUKI_ROOT"/vendor/shirayuki/build/tools/smali/baksmali.jar
        export SMALIJAR="$SHIRAYUKI_ROOT"/vendor/shirayuki/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SHIRAYUKI_ROOT"/vendor/shirayuki/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SHIRAYUKI_ROOT"/vendor/shirayuki/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$SHIRAYUKI_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SHIRAYUKI_TARGET" >/dev/null; then
    ...
    for ARCH in $ARCHES; do
        ...
        elif [[ "$SHIRAYUKI_TARGET" =~ .jar$ ]]; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SHIRAYUKI_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SHIRAYUKI_ROOT"/vendor/shirayuki/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SHIRAYUKI_ROOT"/"$OUTDIR"/radio
